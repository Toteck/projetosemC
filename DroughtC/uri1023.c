...
int main() {
	...
	while(scanf("%d", &n) && n != 0){		
		...
		if(num > 1)
			printf("\n");
		printf("Cidade# %d:\n", num);
		for(c=0;c<n;c++){ // Aqui ele acessa por indices. Percorre a lista por indices em cada caso
			out = Lugares[c].pessoas; // 0
			deu = 0;
			while(c+1 < n && deu == 0){ // Aqui na primeira execução n é igual a 2 e c é igual a 0 por isso ele entre no loop. No nível de objeto isso seria objeto.next
				if(floor(Lugares[c].cmedio) == floor(Lugares[c+1].cmedio)){ // it.cmedio == 
					out += Lugares[c+1].pessoas;
					c++; // talvez eu precise criar um auxiliar para fazer o papel desse c
				}else
					deu = 1;
			}
